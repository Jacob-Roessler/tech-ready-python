...
class player:
    ...
    def draw(self, direction):
        if direction == "left":
            for s in range(self.segments):
                self.x[s] = self.x[0]+block_size*s
                self.y[s] = self.y[0]
        if direction == "right":
            for s in range(self.segments):
                self.x[s] = self.x[0]-block_size*s
        ...
        if direction == "up":
            for s in range(self.segments):
                self.x[s] = self.x[0]
                self.y[s] = self.y[0]+block_size*s
        if direction == "down":
            for s in range(self.segments):
                self.x[s] = self.x[0]
                self.y[s] = self.y[0]-block_size*s
...
def message_to_screen(msg, color):
    ...
    gameDisplay.blit(screen_text, [screen_height/2 - 100, screen_width/2])
...
def gameloop():
    ...
    while not gameExit:
        ...
        while gameOver == True:
            ...
            message_to_screen(f"Game over, C to continue or Q to quit  Score: {p1.segments-3}", black)
